 ...
 * @param key_no 按键号
 */
void smart_greenhouse_key_process(int key_no)
{
    printf("smart_greenhouse_key_process:%d\n",key_no);
    if(key_no == KEY_UP){
 ...
 */
void smart_greenhouse_iot_cmd_process(int iot_cmd)
{
    switch (iot_cmd)
    {
 ...
 */
void smart_greenhouse_su03t_cmd_process(int su03t_cmd)
{
    switch (su03t_cmd)
    {
        case light_state_on:
...
void lcd_show_ui(void)
{
    ...
    lcd_show_string(10, 0, "Smart Greenhouse", LCD_RED, LCD_WHITE, 32, 0);
